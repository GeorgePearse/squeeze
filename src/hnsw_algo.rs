...
impl Hnsw {
    ...
    fn select_neighbors(candidates: &BinaryHeap<Candidate>, m: usize) -> Vec<usize> {
        // Optimized: avoid cloning the heap by iterating and collecting
        // BinaryHeap iteration is not sorted, but we convert to sorted vec first
    ...
    fn prune_connections<F>(node_idx: usize, neighbors: &[usize], m_max: usize, dist_fn: &F) -> Vec<usize>
    where F: Fn(usize, usize) -> f32
    {
