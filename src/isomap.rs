...
impl Isomap {
    ...
    pub fn fit_transform(&self, data: &Array2<f64>) -> Result<Array2<f64>> {
        ...
        let geodesic_distances = self.compute_geodesic_distances(&knn_graph, n_samples)?;

        // Apply classical MDS to geodesic distances
        classical_mds(&geodesic_distances, self.n_components)
    }
}
    ...
    fn compute_geodesic_distances(
        ...
        if has_inf {
            return Err(Error::Computation(
                "graph is disconnected; try increasing n_neighbors".into(),
            ));
        }
...
mod tests {
    ...
    fn test_geodesic_distances_simple_chain() {
        ...
        let geodesic = isomap.compute_geodesic_distances(&knn_graph, 4).unwrap();
    ...
    fn test_floyd_warshall_correctness() {
        ...
        let geodesic = isomap.compute_geodesic_distances(&knn_graph, 3).unwrap();
    ...
    fn test_disconnected_graph_detection() {
        ...
        let result = isomap.compute_geodesic_distances(&knn_graph, 4);
