...
# -----------------------------------------------
def test_nn_bad_metric(nn_data) -> None:
    with pytest.raises(ValueError):
        nearest_neighbors(nn_data, 10, 42, {}, False, np.random)


def test_nn_bad_metric_sparse_data(sparse_nn_data) -> None:
    with pytest.raises(ValueError):
        nearest_neighbors(
            sparse_nn_data,
            10,
...
def smooth_knn(nn_data, local_connectivity=1.0):
    _knn_indices, knn_dists, _ = nearest_neighbors(
        nn_data,
        10,
        "euclidean",
        {},
...
@pytest.mark.skip
def test_nn_descent_neighbor_accuracy(nn_data) -> None:  # pragma: no cover
    knn_indices, _knn_dists, _ = nearest_neighbors(
        nn_data,
        10,
        "euclidean",
        {},
...
@pytest.mark.skip
def test_nn_descent_neighbor_accuracy_low_memory(nn_data) -> None:  # pragma: no cover
    knn_indices, _knn_dists, _ = nearest_neighbors(
        nn_data,
        10,
        "euclidean",
    ...
    percent_correct = knn(knn_indices, nn_data)
    assert percent_correct >= 0.89, (
        "NN-descent did not get 89% accuracy on nearest neighbors"
    )


@pytest.mark.skip
def test_angular_nn_descent_neighbor_accuracy(nn_data) -> None:  # pragma: no cover
    knn_indices, _knn_dists, _ = nearest_neighbors(
        nn_data,
        10,
        "cosine",
...
def test_sparse_angular_nn_descent_neighbor_accuracy(
    ...
    percent_correct = knn(knn_indices, angular_data)
    assert percent_correct >= 0.90, (
        "Sparse NN-descent did not get 90% accuracy on nearest neighbors"
    )


def test_smooth_knn_dist_l1norms(nn_data) -> None:
...
def test_smooth_knn_dist_l1norms_w_connectivity(nn_data) -> None:
