...
(() => {
    ...
    constructor({ config, docs, options }) {
      ...
      this.index = lunr(function() {
        ...
        for (const [name, spec] of Object.entries(config.fields))
          this.field(name, __spreadProps(__spreadValues({}, spec), { extractor: field(name) }));
        for (const doc of docs)
  ...
  function queryLLM(query, context) {
    return __async(this, null, function* () {
      var _a, _b, _c, _d, _e, _f, _g, _h, _i;
      if (!llmConfig) {
        return { answer: "", error: "LLM not configured" };
      }
      try {
        const { provider, endpoint, model, api_key, system_prompt, max_tokens, temperature, site_name } = llmConfig;
        const systemPrompt = system_prompt || "You are a helpful documentation assistant. Answer questions based on the provided context from the documentation. Be concise and accurate.";
        const contextText = (context == null ? void 0 : context.length) ? `
...
${context.join("\n\n")}` : "";
        ...
        if (provider === "openai" || provider === "openai-compatible") {
          ...
          if (!api_key) {
            return { answer: "", error: "API key not configured for OpenAI provider" };
          }
          response = yield fetch(apiEndpoint, {
            method: "POST",
            headers: {
          ...
          if (!response.ok) {
            ...
            return { answer: "", error: `API error: ${response.status} - ${errorText}` };
          }
          const data = yield response.json();
          answer = ((_c = (_b = (_a = data.choices) == null ? void 0 : _a[0]) == null ? void 0 : _b.message) == null ? void 0 : _c.content) || "";
        } else if (provider === "openrouter") {
          const apiEndpoint = endpoint || "https://openrouter.ai/api/v1/chat/completions";
          const modelName = model || "openai/gpt-3.5-turbo";
          if (!api_key) {
            return { answer: "", error: "API key not configured for OpenRouter provider" };
          }
          const headers = {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${api_key}`
          ...
          try {
            headers["HTTP-Referer"] = ((_d = self.location) == null ? void 0 : _d.origin) || "https://mkdocs-material.local";
          } catch (e) {
            headers["HTTP-Referer"] = "https://mkdocs-material.local";
          }
          if (site_name) {
          ...
          const data = yield response.json();
          answer = ((_g = (_f = (_e = data.choices) == null ? void 0 : _e[0]) == null ? void 0 : _f.message) == null ? void 0 : _g.content) || "";
        } else if (provider === "anthropic") {
          const apiEndpoint = endpoint || "https://api.anthropic.com/v1/messages";
          const modelName = model || "claude-3-haiku-20240307";
          if (!api_key) {
            return { answer: "", error: "API key not configured for Anthropic provider" };
          }
          response = yield fetch(apiEndpoint, {
            method: "POST",
            headers: {
          ...
          const data = yield response.json();
          answer = ((_i = (_h = data.content) == null ? void 0 : _h[0]) == null ? void 0 : _i.text) || "";
        } else if (provider === "custom") {
          if (!endpoint) {
            return { answer: "", error: "Custom provider requires an endpoint" };
          }
          ...
          return { answer: "", error: `Unknown LLM provider: ${provider}` };
        }
        return { answer };
      } catch (err) {
        const errorMessage = err instanceof Error ? err.message : "Unknown error";
        return { answer: "", error: `LLM query failed: ${errorMessage}` };
      }
    });
  }
  function handler(message) {
    return __async(this, null, function* () {
      switch (message.type) {
        ...
        /* LLM configuration setup message */
        case 4 /* SETUP_LLM */:
          llmConfig = message.data;
          return {
            type: 1 /* READY */
          };
        /* LLM search query message */
        case 5 /* QUERY_LLM */:
          const llmQuery = message.data.query;
          const context = message.data.context;
          if (llmConfig) {
            console.log("LLM Query with config:", __spreadProps(__spreadValues({}, llmConfig), { api_key: llmConfig.api_key ? "Present" : "Missing" }));
          }
          const llmResult = yield queryLLM(llmQuery, context);
          return {
            type: 6 /* RESULT_LLM */,
            data: llmResult
          };
  ...
  addEventListener("message", (ev) => __async(null, null, function* () {
