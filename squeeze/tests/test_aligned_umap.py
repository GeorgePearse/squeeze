...
def test_neighbor_local_neighbor_accuracy(aligned_iris, aligned_iris_model) -> None:
...
def test_local_clustering(aligned_iris, aligned_iris_model) -> None:
    _data, target = aligned_iris
...
def test_aligned_update(aligned_iris, aligned_iris_relations) -> None:
    data, _target = aligned_iris
...
def test_aligned_update_params(aligned_iris, aligned_iris_relations) -> None:
    data, _target = aligned_iris
...
def test_aligned_update_array_error(aligned_iris, aligned_iris_relations) -> None:
    data, _target = aligned_iris
    n_neighbors = [15, 15, 15, 15, 15]
    small_aligned_model = AlignedUMAP(n_neighbors=n_neighbors[:3])
    small_aligned_model.fit(data[:3], relations=aligned_iris_relations[:2])
